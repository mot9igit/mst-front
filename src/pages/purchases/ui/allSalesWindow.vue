<template>
  ...
  <div v-for="(item, remain_id) in offers" :key="remain_id">
      ...
      <div class="product-card__price-container-all">
        ...
        <div class="product-card__count">
          <div class="product-card__count-value">
            ...
            <span v-else>{{ item.item.available }} шт</span>
              ...
              >
                ...
                <div class="d-switch__circle">
                  ...
                  <div
                    ...
                    v-if="
                      this.mainActionsData[remain_id][item.action_id] == false && !allOff[remain_id]
                    "
                  >
                    !
                  </div>
...
export default {
  ...
  mounted() {
    ...
    if (Object.keys(this.offers).length) {
      for (var rem_id in this.offers) {
        for (var ic in this.offers[rem_id].item.conflicts) {
          if (this.offers[rem_id].item.conflicts[ic].active) {
            this.activeConflict[rem_id] = this.offers[rem_id].item.conflicts[ic]
          }
        }
        if (this.activeConflict[rem_id].actions_ids) {
          let item = JSON.parse(JSON.stringify(this.activeConflict[rem_id].actions_ids))
          for (var iitem in item) {
            item[iitem] = Number(item[iitem])
          }

          let m_a = {}
          for (var ima in this.offers[rem_id].item.main_actions) {
            if (item.includes(Number(this.offers[rem_id].item.main_actions[ima]))) {
              m_a[this.offers[rem_id].item.main_actions[ima]] = true
            } else {
              m_a[this.offers[rem_id].item.main_actions[ima]] = false
            }
          }
          this.mainActionsData[rem_id] = m_a

          if (
            Number(this.activeConflict[rem_id].multiplicity) >
              Number(this.activeConflict[rem_id].min_count) &&
            Number(this.activeConflict[rem_id].multiplicity) > 1
          ) {
            this.counts[rem_id].count = Number(this.activeConflict[rem_id].multiplicity)
            this.counts[rem_id].step = Number(this.activeConflict[rem_id].multiplicity)
            this.counts[rem_id].count_min = Number(this.activeConflict[rem_id].multiplicity)
          } else {
            if (
              Number(this.activeConflict[rem_id].multiplicity) <=
                Number(this.activeConflict[rem_id].min_count) &&
              ...
              if (
                ...
                this.counts[rem_id].count = Number(this.activeConflict[rem_id].min_count)
                this.counts[rem_id].step = Number(this.activeConflict[rem_id].multiplicity)
                this.counts[rem_id].count_min = Number(this.activeConflict[rem_id].min_count)
              } else {
                this.counts[rem_id].count =
                  Number(this.activeConflict[rem_id].min_count) +
                  Number(this.activeConflict[rem_id].multiplicity) -
                  (Number(this.activeConflict[rem_id].min_count) %
                    Number(this.activeConflict[rem_id].multiplicity))
                this.counts[rem_id].step = Number(this.activeConflict[rem_id].multiplicity)
                this.counts[rem_id].count_min = this.counts[rem_id].count
              }
            } else {
              this.counts[rem_id].count =
                Number(this.activeConflict[rem_id].min_count) > 0
                  ? Number(this.activeConflict[rem_id].min_count)
                  : 1
              this.counts[rem_id].step = 1
              this.counts[rem_id].count_min = this.counts[rem_id].count
            }
          }
          ...
          if (
            ...
            if (this.step == 1) {
              this.counts[rem_id].count_min > Number(this.offers[rem_id].item.count)
                ? (this.counts[rem_id].count = this.counts[rem_id].count_min)
                : (this.counts[rem_id].count = Number(this.offers[rem_id].item.count))
            } else {
              this.counts[rem_id].count < Number(this.offers[rem_id].item.count)
                ? (this.counts[rem_id].count = Number(this.offers[rem_id].item.count))
                : ''
          ...
          this.$emit('counter', {
            item: this.offers[rem_id].item,
            count: this.counts[rem_id].count,
          })
        }
      }
    }
  ...
  methods: {
    ...
    //   this.counts[object.item.remain_id].count = object.value
    //   if (
    //     this.$route.matched[5] &&
    //     this.$route.matched[5].name == 'purchasesCatalogRequirement'
    //   ) {
    //     this.$emit('counter', { item: object.item, count: this.counts[object.remain_id].count })
    //   }
    //   return
    // }
    ...
    // } else {
    //   this.counts[object.remain_id].count = object.value
    //   if (
    //     this.$route.matched[5] &&
    //     this.$route.matched[5].name == 'purchasesCatalogRequirement'
    //   ) {
    //     this.$emit('counter', { item: object.item, count: this.count })
    //   }
    // }
    // if (object.value < object.min) {
    //   this.counts[object.remain_id].count = this.counts[object.remain_id].count_min
    //   if (
    //     this.$route.matched[5] &&
    //     this.$route.matched[5].name == 'purchasesCatalogRequirement'
    //   ) {
    ...
    //   this.counts[rem_id].count_min = Number(this.activeConflict.multiplicity)
    // } else {
    //   if (
    //     Number(this.activeConflict.multiplicity) <= Number(this.activeConflict.min_count) &&
    //     Number(this.activeConflict.multiplicity) > 1
    ...
    //       this.step = Number(this.activeConflict.multiplicity)
    //       this.counts[rem_id].count_min = Number(this.activeConflict.min_count)
    //     } else {
    //       this.count =
    ...
    //       this.step = Number(this.activeConflict.multiplicity)
    //       this.counts[rem_id].count_min = this.count
    //     }
    //   } else {
    //     this.count =
    ...
    //   if (this.step == 1) {
    //     this.counts[rem_id].count_min > Number(this.offer.count)
    //       ? (this.count = this.counts[rem_id].count_min)
    ...
    //   if (this.mainActionsData[r_id][ind]) {
  ...
  watch: {
    ...
    //   for (var r_id in newVal) {
    //     if (newVal[r_id] == false) {
    //       if (!this.allOff[r_id]) {
    //         if (
    //           Number(this.activeConflict[r_id].multiplicity) <= 1 ||
    //           Number(this.activeConflict[r_id].min_count) <= 1
    //         ) {
    //           // потребность
    //           if (
    //             this.$route.matched[5] &&
