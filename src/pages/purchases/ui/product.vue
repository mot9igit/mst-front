...
import productInfoWindow from './productInfoWindow.vue'
  ...
  mounted() {
    if (Object.keys(this.product).length) {
      if (this.product.id != 0 && this.product.id != null) {
        if (
          this.product?.category_characters?.length > 1 ||
          this.product?.category_characters?.length > 0
        ) {
          this.showInfo = true
        }
      }
...
.product-modal__info .swiper-button-prev.swiper-button-disabled,
.product-modal__info .swiper-button-next.swiper-button-disabled {
