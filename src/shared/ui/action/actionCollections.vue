              ...
              <table class="d-table d-table--decoration" v-if="el.products">
                <thead class="d-table__head">
                  <tr class="d-table__row">
                    ...
                    <th class="d-table__head-col d-table__head-col--big">Цена со скидкой за шт.</th>
                  </tr>
                </thead>
                <tbody class="d-table__body promotions__product-card">
                  ...
                  <tr class="d-table__row" v-for="product in el.products?.items" :key="product.id">
...
export default {
  ...
  emits: ['filterCollections', 'paginateGroup', 'selectCollection', 'deleteCollection'],
  components: { Paginate, TabView, TabPanel },
  props: {
  ...
  computed: {
    pagesCount() {
      if (this.collections.products) {
        let pages = Math.ceil(this.collections.products.total / this.perPage)
        if (pages === 0) {
          pages = 1
        }
        return pages
  ...
  watch: {
    search(newVal) {
      ...
      this.debounce(() => {
        this.$emit('filterCollections', this.search)
      }, 300)
    },
  },
}
