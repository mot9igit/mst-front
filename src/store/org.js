...
export default {
  ...
  actions: {
    ...
    async getFiltersStores({ commit }, { name }) {
      const data = {
        action: 'get/stores',
        id: router.currentRoute._value.params.id,
        mode: 'all',
      ...
      const response = await api.org.getOrg(data)
      if (response) {
        commit('SET_FILTERS_STORES', response.data)
      }
      return response
    },

    async getOrgStoresAll({ commit }, { page, perpage, filters }) {
      const data = {
        action: 'get/stores',
        id: router.currentRoute._value.params.id,
        page: page,
      ...
      const response = await api.org.getOrg(data)
      if (response) {
        commit('SET_ORG_STORES_ALL', response.data)
      }
      return response
    },
  ...
  mutations: {
    ...
    SET_FILTERS_STORES: (state, data) => {
      state.filterStores = data.data.items
    },
    SET_ORG_STORES_ALL: (state, data) => {
      state.orgStoresAll = data.data
    },
  ...
  getters: {
    ...
    orgStoresAll(state) {
      return state.orgStoresAll
    },
    filterStores(state) {
      return state.filterStores
    },
    storeSettings(state) {
